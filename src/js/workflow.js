...
function parse_routing(text) {
	...
	for (const line of sec[0].split(/\r?\n/)) {
		...
		if (pat === '(default)') { rows.push({ default: true, workflow: wf }); continue; }
...
export function load_workflow() {
	...
	cache = wf;
	try { cache_mtime = statSync(path).mtimeMs; } catch {}
	return wf;
}

export function get_workflow_for(question) {
...
export function should_auto_enqueue() {
...
export function bias_by_focus(text) {

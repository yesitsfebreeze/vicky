...
const sh_async = (cmd, opts = {}) => new Promise(
...
async function query_graph(question, graph = fs.kb_graph(), prefix = null) {
...
async function query_graph_hits(question, prefix = null, graphPath = fs.kb_graph(), limit = 10) {
...
function list_titles_from_graph(graphPath = fs.kb_graph(), prefix = null) {
  if (!existsSync(graphPath)) return [];
  try {
    const { nodes } = JSON.parse(readFileSync(graphPath, "utf8"));
    return [...new Set(
